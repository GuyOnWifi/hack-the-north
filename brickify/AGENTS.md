...
19. **Records are repaired when read, never trusted.** `pipeline.record()`
    rebuilds a run's paths from its folder, so runs outlive the code that
    wrote them and a model made on one laptop opens on another. The library,
    the replay and the review all read through it.
20. **The gallery is how models travel.** `brickify.share export/import` moves
    starred runs through the repo, slimmed to the winning model and its
    pictures (~400KB each). `runs/` itself stays local.
