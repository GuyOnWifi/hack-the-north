...
export function SketchPad({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      {/* flex column capped to the viewport: header + footer NEVER scroll off, the
          canvas takes whatever height is left — so the X and Build are always reachable */}
      <div className="flex max-h-[92vh] w-full max-w-[440px] flex-col rounded-[24px] bg-white p-4 shadow-[0_20px_60px_rgba(0,0,0,0.4)]" onClick={(e) => e.stopPropagation()}>
        <div className="flex shrink-0 items-center justify-between">
          <h2 className="text-[19px] font-[800] text-ink">Sketch it</h2>
          <button onClick={onClose} aria-label="Close" className="grid h-9 w-9 place-items-center rounded-full text-ink hover:bg-black/5 active:scale-95">
            <X size={22} strokeWidth={2.4} />
          </button>
        </div>
        <p className="mt-1 shrink-0 text-[14px] text-ink-soft">Draw a rough shape — the builder matches your silhouette.</p>
        <div className="mt-3 min-h-[160px] flex-1">
          <canvas
            ...
            onPointerUp={stop}
            onPointerLeave={stop}
            className="h-full w-full touch-none rounded-[16px] border-2 border-[#e4e4e4] bg-white"
          />
        </div>
        ...
        <ChunkyButton variant="blue" onClick={build} disabled={!hasInk || busy} className="mt-3 w-full shrink-0 !text-[16px]">
